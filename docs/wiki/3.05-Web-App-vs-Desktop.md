...
In the web version, time tracking only works while the app is open. This fork does not detect idle time or ask how to allocate it. Timers run until stopped manually; while a task is tracked the browser tab title shows the time logged on it today and its name (for example `25m - Write report`), which keeps a running timer visible from a background tab.

The desktop task widget uses the same project and Today task data as the main window. Its Today list includes open tasks due today and open overdue tasks shown under “From previous days” in the main window. Imported AI elapsed and estimated review durations are visible on each widget row when available.
...
Jira integration in the web version requires the Super Productivity Chrome Extension to be installed.
