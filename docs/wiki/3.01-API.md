...
### Activity totals (local fork)

The authenticated local REST API accepts `PUT /tasks/:id/activity` with
`{source, date, humanMs, aiMs, inProgress?}`. `date` is a calendar date (`YYYY-MM-DD`),
`source` is a stable collector identifier, and durations are cumulative integer
milliseconds for that source/task/day.
Parent tasks can also receive their own observation receipt. A parent receipt
updates its notes but not its native day-work log, because that log is derived
from subtasks. Do not duplicate a child's observed time on its parent.

Repeated identical totals are idempotent. Corrections apply only the difference
from the previous human total, preserving other tracked time. The receipt and
native day-work-log update are one ordinary persistent task update. Receipts are
stored in a `sp-activity-v1` HTML comment in task notes for compatibility with
unmodified clients; preserve that comment when editing notes. AI totals stay in
receipts rather than inflating personal work logs. `inProgress: true` marks a task
in progress only when new observations arrive; an identical retry does not undo a
manual status change. This endpoint does not start the native timer.
