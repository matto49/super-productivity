...
### Imported task activity

This fork stores imported AI elapsed time and estimated human review time as source-and-day receipts in task notes. The task list and task detail panel show these values separately from the built-in timer and estimate; they are not added together as a single work duration. Existing receipts remain in normal task backups and sync data even if the external collector is unavailable.
