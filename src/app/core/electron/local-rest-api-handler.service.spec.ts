...
describe('LocalRestApiHandlerService', () => {
  ...
  describe('task routes', () => {
    ...
    describe('PUT /tasks/:id/activity', () => {
      it('imports native day time and its receipt in one update', async () => {
        const mockTask = createMockTask('task-1');
        Object.defineProperty(taskServiceMock, 'getByIdOnce$', {
          get: () => (_id: string) => of(mockTask),
        });
        const response = await sendRequestAndWait(
          createRequest('PUT', '/tasks/task-1/activity', {
            body: { source: 'mac', date: '2026-09-07', humanMs: 120000, aiMs: 300000 },
          }),
        );
        expect(response.body.ok).toBe(true);
        expect(taskServiceMock.update).toHaveBeenCalledTimes(1);
        const changes = taskServiceMock.update.calls.mostRecent().args[1];
      ...
      it('rejects negative durations before any writes', async () => {
        ...
        const response = await sendRequestAndWait(
          createRequest('PUT', '/tasks/task-1/activity', {
            body: { source: 'mac', date: '2026-09-07', humanMs: -1, aiMs: 0 },
          }),
        );
        expect(response.body.ok).toBe(false);
