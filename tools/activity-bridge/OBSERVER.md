...
6. Separately refresh activity totals with `python3 tools/activity-bridge/bridge.py --config /Users/bytedance/todo-review-20260907/activity/config.json --ai-only --apply`. The Mac collectors alone write the private human ledger; the devbox observer imports it with AI totals. Never read raw History over SSH or treat an AI-only import as proof that a human source is running. If the existing ledger is missing, report the error rather than resetting human time. Read `human-collection.json` for Computer History status and the bridge report's `foregroundCollection` for the separate macOS foreground sampler. `foregroundCollection=observing` means its own recent heartbeat is live; it does not mean Computer History recording is running or that any Todo matched. Report the two sources separately, including stale/permission-required states and the number of new foreground intervals. Explicit conversation acceptance can still be evaluated.
